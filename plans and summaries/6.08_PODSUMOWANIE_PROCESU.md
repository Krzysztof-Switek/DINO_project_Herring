...
## 35. Gdzie jesteśmy teraz — podsumowanie liczbowe
...
- **WDROŻONE, NIEURUCHOMIONE (26.08, Etap 32): Opcja A (pół-słaby nadzór) — kod gotowy, pełny bieg
  serwerowy jeszcze nie odpalony.** `zegar_position_weight` w głównym pipeline, BEZ kanonizacji osi
  (uzasadnienie: `RadialAttentionDensityHead` już ma własne kodowanie pozycyjne per-obrazowe, w
  przeciwieństwie do głowicy Opcji B). 30/42 obrazów ZEGAR dostaje nadzór pozycyjny, 12 całkowicie
  wykluczonych z treningu dla uczciwej ewaluacji post-hoc. 455/455 testów zielonych (13 nowych),
  smoke test 7/7. Następny krok (poza tą sesją): uruchomienie `main_zegar_semi_weak.py` na
  serwerze, potem re-scoring held-out metodą `expert_annotation_eval.py`.
