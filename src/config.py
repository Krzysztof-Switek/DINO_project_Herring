...
class ModelConfig(BaseModel):
    ...
    # Semi-weak supervision (26.08, "Opcja A"): weight of an ADDITIONAL loss term
    # (zegar_position_loss, src/model.py) pulling the density head toward REAL
    # human-annotated ring positions, for the small subset of batch samples that have
    # them (the 42-image "ZEGAR" ground-truth set, wired in via OtolithDataset's
    # zegar_heatmap/has_zegar_target — see data.zegar_* fields below). Every other
    # sample keeps using density_count_loss's purely weak (age-count-only) supervision
    # unchanged. 0.0 = off (default) — dataset pays zero extra cost and no batch key
    # is required. JAWNIE NIEZWERYFIKOWANY punkt startowy, same status as every other
    # first-attempt weight in this project (e.g. density_attn_window_deg on 15.08).
    zegar_position_weight: float = Field(0.0, ge=0.0)
...
class DataConfig(BaseModel):
    ...
    # lives OUTSIDE the main labels_csv/image_dir (regenerated by scripts/prepare_labels.py
    # on every RESCAN=True run, which would silently drop any ZEGAR rows written directly
    # into it — see scripts/prepare_zegar_semi_weak_data.py's module docstring). All four
    # None by default = zero behaviour change for every existing config.
    zegar_extra_labels_csv: Optional[str] = None    # image_id,age,split rows (split="train" only)
    zegar_extra_image_dir: Optional[str] = None     # processed ZEGAR crops, separate from image_dir
