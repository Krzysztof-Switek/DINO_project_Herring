"""15.08 — Strojenie hiperparametru: szersze lokalne okno uwagi w RadialAttentionDensityHead
(density_attn_window_deg: 45° → 90°), jedyna zmiana względem Run N (config_radial_attention.yaml).

Po dwóch tanich diagnostykach 15.08 (bez treningu): hipoteza artefaktu wysokonormowych neuronów
backbone'u WYKLUCZONA (korelacja norma↔density ~0), a sweep rozdzielczości density przy
wnioskowaniu dał słaby, niemonotoniczny sygnał — nieuzasadniający treningu z wyższą
rozdzielczością w tym tygodniu. Zamiast tego: pierwsza ablacja hiperparametrów samej architektury
radial_attention, dotąd zero testowana. Pełne uzasadnienie w nagłówku
configs/config_radial_attention_wide_window.yaml i
plans and summaries/15.08_radial_attention_b_PODSUMOWANIE_I_PLAN.md — przeczytaj PRZED odpaleniem.

Osobny plik od main_radial_attention.py/main_radial_attention_b.py — nie nadpisuje żadnego
...
RUN_TAG = datetime.now().strftime("%d.%m") + "_radial_attention_wide_window"
OUTPUT_DIR = str(PROJECT_ROOT / "outputs" / "data" / RUN_TAG)
BASE_CONFIG = str(PROJECT_ROOT / "configs" / "config_radial_attention_wide_window.yaml")
...
if __name__ == "__main__":
    ...
    if not Path(IMAGE_DIR).is_dir():
        sys.exit(
            f"[main_radial_attention_wide_window] Katalog zdjęć nie istnieje: {IMAGE_DIR!r}\n"
            f"       LOCATION = {LOCATION!r} — sprawdź czy to właściwa maszyna,\n"
            f"       albo popraw IMAGE_DIR_SERVER / IMAGE_DIR_LOCAL powyżej."
        )
    print(f"[main_radial_attention_wide_window] LOCATION={LOCATION}  IMAGE_DIR={IMAGE_DIR}  RESCAN={RESCAN}")
    print(f"[main_radial_attention_wide_window] BASE_CONFIG={BASE_CONFIG}")
    print(f"[main_radial_attention_wide_window] OUTPUT_DIR={OUTPUT_DIR}")
    main(ARGV)
