"""10.08 — Zmiana A v2: kodowanie pozycyjne biegunowe + lokalna uwaga w głowicy density.

Po zamknięciu macierzy izolacji A×B (Run K / isolate_a / isolate_b — density_active
=0,0000 w KAŻDYM biegu ze zwykłą architekturą uwagi, niezależnie od Zmiany B): świadomie
WIĘKSZY, skonsolidowany krok zamiast kolejnego małego testu izolacyjnego (decyzja
użytkownika 10.08). Łączy jawne kodowanie pozycyjne (t,θ) + uwagę ograniczoną lokalnie
(promień×kąt) — oba umotywowane niezależnie: przeglądem kodu (08.08) i literaturą
(RadialFormer, 09.08). Pełne uzasadnienie w nagłówku configs/config_radial_attention.yaml
— przeczytaj PRZED odpaleniem.

Osobny plik od main.py/main_e9.py/main_attention_first.py/main_isolate_a.py/
main_isolate_b.py — nie nadpisuje żadnego dotychczasowego biegu/configu. Kliknij ▶ na
serwerze.
"""
...
RUN_TAG = datetime.now().strftime("%d.%m") + "_radial_attention"
OUTPUT_DIR = str(PROJECT_ROOT / "outputs" / "data" / RUN_TAG)
BASE_CONFIG = str(PROJECT_ROOT / "configs" / "config_radial_attention.yaml")
...
if __name__ == "__main__":
    ...
    if not Path(IMAGE_DIR).is_dir():
        sys.exit(
            f"[main_radial_attention] Katalog zdjęć nie istnieje: {IMAGE_DIR!r}\n"
            f"       LOCATION = {LOCATION!r} — sprawdź czy to właściwa maszyna,\n"
            f"       albo popraw IMAGE_DIR_SERVER / IMAGE_DIR_LOCAL powyżej."
        )
    print(f"[main_radial_attention] LOCATION={LOCATION}  IMAGE_DIR={IMAGE_DIR}  RESCAN={RESCAN}")
    print(f"[main_radial_attention] BASE_CONFIG={BASE_CONFIG}")
    print(f"[main_radial_attention] OUTPUT_DIR={OUTPUT_DIR}")
    main(ARGV)
