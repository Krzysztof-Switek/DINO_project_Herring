"""29.07: side-by-side local comparison report — `22.07_reg` (bez prioru E9) vs
`28.07_e9_w0.1` (Faza 1, WARIANT A: density_concentricity_weight=0.1), SAME otoliths,
same panels, laid out next to each other so a human can visually judge whether the
radial-concentricity prior changes the density heatmap (bardziej pierścieniowa / mniej
rozproszona) beyond the aggregate pixel-distance metrics in localization_quality.json.

Modelled 1:1 on scripts/diagnostics/compare_22reg_vs_hires966.py (28.07) — same wiring,
only RUN_B / LABEL_B / config swapped. Does NOT retrain or modify any src/ code. Reuses:
  - scripts.run_pipeline.load_merged_config / _compute_axis_data_for_samples / _parse_train_log
  - src.visualization.draw_reasoning_card / load_original_image
  ...
  - the already-computed 30-card localization_quality.json for both runs
    (outputs/23.07_reg_30cards, outputs/28.07_e9_w0.1/localization_quality.json) — no
    recomputation.

Usage: python scripts/diagnostics/compare_22reg_vs_e9_w0.1.py
"""
...
RUN_B_DIR = PROJECT_ROOT / "outputs" / "28.07_e9_w0.1"
CKPT_A = RUN_A_DIR / "checkpoints" / "embedded" / "best.pt"
CKPT_B = RUN_B_DIR / "checkpoints" / "embedded" / "best.pt"
LABEL_A, LABEL_B = "22.07_reg (bez E9)", "28.07_e9_w0.1 (E9, w=0.1)"
COLOR_A, COLOR_B = "#2a78d6", "#2fa66a"

OUT_DIR = PROJECT_ROOT / "outputs" / "29.07_compare_22reg_vs_e9_w0.1"
...
def main() -> None:
    ...
    cfg_b = load_merged_config(PROJECT_ROOT / "configs" / "config_e9_w0.1.yaml",
                               PROJECT_ROOT / "configs" / "config_embedded.yaml")
    ...
    print("Liczenie run A (22.07_reg, bez E9)...", flush=True)
    grids_a, axis_data_a, _ = _compute_axis_data_for_samples(
        samples_a, IMAGE_DIR, cfg_a, CKPT_A, OUT_DIR / "run_a")
    print("Liczenie run B (28.07_e9_w0.1, z E9)...", flush=True)
    ...
    q_b = json.loads((RUN_B_DIR / "localization_quality.json").read_text(encoding="utf-8"))["emb_on_emb"]
    ...
    ax2.plot([r["epoch"] for r in log_b], [r["val_mae"] for r in log_b],
             color=COLOR_B, marker="o", markersize=3, label=LABEL_B, linestyle="--")
    ...
    ax2.set_title("Krzywe treningowe — val_mae per epoka (bramka bezpieczeństwa: powinny się pokrywać)")
...
<title>29.07 — 22.07_reg (bez E9) vs 28.07_e9_w0.1 (z E9): te same otolity, obok siebie</title>
<style>
...
<body>
<h1>22.07_reg (bez E9) vs 28.07_e9_w0.1 (z E9, waga=0.1) — te same otolity, obok siebie</h1>
<p class="cap">Oba biegi: ta sama receptura (backbone <code>dinov2_vits14_reg</code>, 518px, ten sam test set,
identyczny split) — JEDYNA różnica: <code>density_concentricity_weight</code> (0.0 w run A, 0.1 w run B,
WARIANT A Fazy 1 — patrz <code>plans and summaries/29.07_session_handoff.md</code>). Checkpointy:
...
dobranych deterministycznie (bez cherry-pickingu po błędzie żadnego z modeli).</p>

<h2>0. Bramka bezpieczeństwa</h2>
<p class="cap">Strata E9 jest stop-gradient (nie dotyka backbone/CORAL/MIL) — krzywa val_mae MUSI się
pokrywać między biegami. Sprawdzone też liczbowo w train.log (patrz sekcja 3 niżej): epoka-po-epoce
identyczne do ~4. miejsca po przecinku, drobne różnice rzędu 1e-3 to typowy szum GPU non-determinizmu,
NIE regresja wieku.</p>

<h2>1. Wiek (CORAL) — test set, n={len(pred_a)}</h2>
...
kontrola o run-to-run szum (klasyczny sygnał, w teorii deterministyczny funkcją obrazu, w praktyce
przesuwa się o kilka % między niezależnymi uruchomieniami pipeline'u — patrz selekcja "final" w
<code>fuse_increments</code>, zależna też od <code>predicted_age</code>). Traktuj różnice rzędu
tego szumu jako brak sygnału. Surowe wartości px: powyższy wykres.</p>
