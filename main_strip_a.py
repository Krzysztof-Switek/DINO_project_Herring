"""02.09 — Eksperyment "pasek dendrologiczny" (Track A, dwugałęziowy): wiek na całym obrazie,
...
Pełne uzasadnienie: plans and summaries/02.09_wycinki_plan.md, nagłówek configs/config_strip_a.yaml
— przeczytaj PRZED odpaleniem.

Osobny plik od main_radial_attention.py i innych — nie nadpisuje żadnego dotychczasowego
...
RUN_TAG = datetime.now().strftime("%d.%m") + "_strip_a"
OUTPUT_DIR = str(PROJECT_ROOT / "outputs" / "data" / RUN_TAG)
BASE_CONFIG = str(PROJECT_ROOT / "configs" / "config_strip_a.yaml")
...
if __name__ == "__main__":
    ...
    if not Path(IMAGE_DIR).is_dir():
        sys.exit(
            f"[main_strip_a] Katalog zdjęć nie istnieje: {IMAGE_DIR!r}\n"
            f"       LOCATION = {LOCATION!r} — sprawdź czy to właściwa maszyna,\n"
            f"       albo popraw IMAGE_DIR_SERVER / IMAGE_DIR_LOCAL powyżej."
        )
    print(f"[main_strip_a] LOCATION={LOCATION}  IMAGE_DIR={IMAGE_DIR}  RESCAN={RESCAN}")
    print(f"[main_strip_a] BASE_CONFIG={BASE_CONFIG}")
    print(f"[main_strip_a] OUTPUT_DIR={OUTPUT_DIR}")
    print("[main_strip_a] UWAGA: karty report.html (density/kandydaci) NIE są świadome gałęzi "
          "paska — ufać tylko predictions.csv/pipeline_summary.json (wiek); lokalizacja liczona "
          "osobno przez scripts/diagnostics/expert_annotation_eval_strip.py.")
    main(ARGV)
